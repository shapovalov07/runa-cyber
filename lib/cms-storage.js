...
const GALLERY_CLUB_CITY_SLUGS = clubCities
  .map((city) => (typeof city?.slug === 'string' ? city.slug.trim().toLowerCase() : ''))
  .filter(Boolean);
const GALLERY_CLUB_CITY_SLUG_SET = new Set(GALLERY_CLUB_CITY_SLUGS);
...
const DEFAULT_TOURNAMENT_EVENTS = [
  {
    ...
    content: 'Регистрация открыта в сообществах RUNA. Подходит для новичков и опытных игроков.',
  ...
  },
  {
    ...
    content: 'Командный сценарий под длительные игровые сессии и тренировочный ритм.',
  ...
  },
  {
    ...
    content: 'Следите за расписанием этапов и форматом участия в клубных анонсах.',
...
const parseGalleryCitySlug = (value, section) => {
  if (section !== 'clubs') return '';

  const slug = getText(value).toLowerCase();
  if (!slug) return '';

  return GALLERY_CLUB_CITY_SLUG_SET.has(slug) ? slug : null;
};
const normalizeGalleryCitySlug = (value, section) => {
  const parsedSlug = parseGalleryCitySlug(value, section);
  return parsedSlug === null ? '' : parsedSlug;
};
...
const normalizeGalleryItem = (item) => {
  const section = normalizeGallerySection(item?.section);
  ...
  return {
    ...
    section,
    citySlug: normalizeGalleryCitySlug(item?.citySlug, section),
    createdAt: toIsoDate(item?.createdAt),
  };
};
...
export { GALLERY_CLUB_CITY_SLUGS, GALLERY_SECTIONS };

export function isGallerySection(value) {
  return Boolean(parseGallerySection(value));
}

export function isGalleryClubCitySlug(value) {
  return Boolean(parseGalleryCitySlug(value, 'clubs'));
}
...
export async function getGalleryPhotos(section, options = {}) {
  ...
  const requestedCitySlug = getText(options?.citySlug).toLowerCase();
  const hasCityFilter = Boolean(requestedCitySlug);
  ...
  const available = (
    await Promise.all(
      normalized.map(async (item) => ({
        item,
        available: await hasManagedGalleryUploadFile(item.src),
      }))
    )
  )
  ...
  if (!hasSectionFilter) {
    if (!hasCityFilter) return available;
    if (!isGalleryClubCitySlug(requestedCitySlug)) return [];
    return available.filter((item) => item.section === 'clubs' && item.citySlug === requestedCitySlug);
  }
  if (!selectedSection) return [];

  const filteredBySection = available.filter((item) => item.section === selectedSection);
  if (!hasCityFilter) return filteredBySection;
  if (selectedSection !== 'clubs' || !isGalleryClubCitySlug(requestedCitySlug)) return [];

  return filteredBySection.filter((item) => item.citySlug === requestedCitySlug);
}
...
export async function getTournamentEventById(id) {
  const eventId = getText(id);
  if (!eventId) return null;

  const list = await getTournamentEvents();
  return list.find((item) => item.id === eventId) ?? null;
}
...
export async function addTournamentEvent(payload) {
  ...
  if (!title || !summary || !imageSrc) {
    throw new Error('Заполните обязательные поля: название, краткое описание и медиа.');
  }
...
export async function updateTournamentEvent(id, payload) {
  const eventId = getText(id);
  if (!eventId) return null;
  ...
  const nextTitle = hasOwn(payload, 'title') ? getText(payload?.title) : current.title;
  const nextSummary = hasOwn(payload, 'summary') ? getText(payload?.summary) : current.summary;
  const nextImageSrc = hasOwn(payload, 'imageSrc') ? getText(payload?.imageSrc) : current.imageSrc;

  if (!nextTitle || !nextSummary || !nextImageSrc) {
    throw new Error('Заполните обязательные поля: название, краткое описание и медиа.');
  }

  const nextItem = normalizeTournamentEventItem({
    ...current,
    title: nextTitle,
    summary: nextSummary,
    content: hasOwn(payload, 'content') ? getText(payload?.content) : current.content,
  ...
  const nextList = [...list];
  nextList[index] = nextItem;
  nextList.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  await writeJsonArray(TOURNAMENT_EVENTS_FILE, nextList);
...
export async function addGalleryPhoto(payload) {
  ...
  const citySlug = parseGalleryCitySlug(payload?.citySlug, section);
  ...
  if (citySlug === null) {
    throw new Error('Выберите корректный город для фото в разделе «Наши клубы».');
  }
...
const hasManagedGalleryUploadFile = async (src) => {
  if (!isManagedGalleryUpload(src)) return true;
