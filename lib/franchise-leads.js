...
const getInteger = (value, fallback = 0) => {
...
const normalizeIntegrationState = (value, fallback = 'pending') => ({
  ...
  attemptedAt: getText(value?.attemptedAt) ? toIsoDate(value.attemptedAt) : '',
  attempts: Math.max(0, getInteger(value?.attempts, 0)),
  externalId: getText(value?.externalId),
...
export async function createFranchiseLead(payload) {
  ...
  if (!name || !city || !phone || !email) {
    throw new Error('Заполните обязательные поля: Имя, Город, Телефон и Email.');
  }
  ...
  const lead = normalizeLead({
    ...
    managerNote: '',
    integrations: {
      telegram: { status: 'pending', sentAt: '', attemptedAt: '', attempts: 0, externalId: '', error: '' },
      email: { status: 'pending', sentAt: '', attemptedAt: '', attempts: 0, externalId: '', error: '' },
      bitrix: { status: 'pending', sentAt: '', attemptedAt: '', attempts: 0, externalId: '', error: '' },
    },
  });
...
export async function getFranchiseLeadById(id) {
...
export async function updateFranchiseLeadIntegrations(id, patch) {
  ...
  const mergeIntegration = (currentState, nextPatch) => {
    const patchValue = nextPatch || {};
    const nextAttemptsRaw = getInteger(patchValue?.attempts, NaN);
    const hasPatch = Object.keys(patchValue).length > 0;

    return {
      ...currentState,
      ...patchValue,
      attemptedAt: getText(patchValue?.attemptedAt) || (hasPatch ? new Date().toISOString() : currentState.attemptedAt),
      attempts: Number.isInteger(nextAttemptsRaw) ? nextAttemptsRaw : currentState.attempts + (hasPatch ? 1 : 0),
    };
  };
  ...
  const nextLead = normalizeLead({
    ...
    integrations: {
      telegram: mergeIntegration(currentLead.integrations.telegram, patch?.telegram),
      email: mergeIntegration(currentLead.integrations.email, patch?.email),
      bitrix: mergeIntegration(currentLead.integrations.bitrix, patch?.bitrix),
    },
  });
